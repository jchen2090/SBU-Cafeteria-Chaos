...
export const GameOverScreen = () => {
  ...
  return (
    <div className="w-full h-full flex-col text-center bg-[#ffc5e6] text-white p-8">
      ...
      <p id="final-score" className="text-8xl font-bold text-[#ff257e] mb-8">
        {state.currentScore}
      </p>
      <div className="grid grid-cols-2 gap-4 text-2xl mb-8 max-w-4xl w-full m-auto">
        <div className="bg-[#f49dc0] p-4 rounded-lg">
          <p className="font-bold text-green-600">Orders Fulfilled</p>
          <p id="orders-fulfilled" className="text-5xl font-bold">
            {state.ordersFulfilled}
          </p>
        </div>
        <div className="bg-[#f49dc0] p-4 rounded-lg">
          <p className="font-bold text-red-600">Orders Lost</p>
      ...
      <div className={`w-full max-w-2xl mx-auto ${renderButtons ? "hidden" : ""}`}>
        <p className="text-3xl text-[#ff257e] font-bold mb-2">🎉 New High Score! 🎉</p>
        <p className="text-xl mb-4">Enter your initials:</p>
        <div className="flex items-center justify-center gap-2 mb-4">
          <div className="w-20 h-24 bg-[#f49dc0] text-6xl font-bold flex items-center justify-center rounded-lg border-4 border-[#e894b5]">
            {initials[0] ?? ""}
          </div>
          <div className="w-20 h-24 bg-[#f49dc0] text-6xl font-bold flex items-center justify-center rounded-lg border-4 border-[#e894b5]">
            {initials[1] ?? ""}
        ...
        <div className="grid grid-cols-10 gap-2 text-2xl font-bold">
          {KEYS.map((key) => (
            <button className="bg-[#f49dc0] hover:bg-[#e894b5] p-4 rounded-lg" onClick={() => handleKeyPress(key)}>
              {key}
            </button>
          ))}
        </div>
        <button
          ...
          className={`mt-4 bg-[#ff2644] hover:bg-[#d6213a] text-white py-2 px-8 rounded-full text-xl ${
      ...
      {renderButtons && (
        <div className="flex justify-center gap-4 mt-8">
          <button
            id="play-again-btn"
            className="bg-[#ff2644] hover:bg-[#d6213a] text-white font-bold text-2xl py-6 px-12 rounded-full transition-transform transform hover:scale-105 shadow-lg border-4 border-[#d6213a] cursor-pointer"
            onClick={playAgain}
          >
            Play Again
          </button>
